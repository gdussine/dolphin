...
public class RESTManager {
    static final String HOST_NAME = "dolphin.jump-technology.com";
    static final int PORT = 8443;


    static final String SCHEME = "https";
    static final String URL = SCHEME + "://" + HOST_NAME + ":" + PORT + "/api/v1/";

    static final int ID_PTF_USER_1 = 1837;
    static final String USERNAME_USER1 = "ESPRIT_GROUPE2";
    static final String PASSWORD_USER1 = Context.get().USER_AUTH_PASS;

    static final int ID_PTF_REF = 2201;

    /**
     * Date de début de la période
     */
    public static String PERIOD_START_DATE = Context.get().PTF_DATE_START;
    /**
     * Date de fin de la période
     */
    public static String PERIOD_END_DATE = Context.get().PTF_DATE_STOP;
    /**
     * Nombre d'actifs du portefeuille
     */
    private static int NB_ASSETS = 20;
    private static int NB_ASSETS_MIN = 20;
    private static int NB_ASSETS_MAX = 40;
    /**
    ...
    public RESTManager(){
        this(RESTManager.HOST_NAME, RESTManager.PORT, Context.get().USER_AUTH_NAME, Context.get().USER_AUTH_PASS);
    }
    ...
    public List<Asset> getAssetList(String parDate) {
        ...
        try {
            ...
            locURIBuilder.addParameter("columns", "CURRENCY");
    ...
    public Double getChangeRate(@Nonnull String parDate,
        ...
        Double locChangeRate = null;
        final ChangeRate changeRate = (ChangeRate) convertJSONtoType(locResponse, TypeToken.getParameterized(ChangeRate.class).getType());
        if (changeRate != null) {
            locChangeRate = Double.valueOf(changeRate._rate._value.replace(',', '.'));
            System.out.println("Change rate " + parCurrencySrc + " -> " + parCurrencyDest + " = " + locChangeRate);
        }
    ...
    public boolean checkPotfolioConditions(@Nonnull Portfolio parPortfolio, @Nonnull AssetCurrencyManager parCurrencyManager) {
        ...
        final List<DynAmountLineContainerModel> locCompo = locListCompo.values().iterator().next();
        if (locCompo == null) {
            System.out.println(
                    String.format("la composition ne date pas du %s", PERIOD_START_DATE));
            return false;
        }
        if (locCompo.size() < NB_ASSETS_MIN) {
            System.out.println(
                    String.format("la composition à un nombre de lignes < de %d", NB_ASSETS_MIN));
            return false;
        }

        if (locCompo.size() > NB_ASSETS_MAX) {
            System.out.println(
                    String.format("la composition à un nombre de lignes > de %d", NB_ASSETS_MAX));
            return false;
        }
        ...
        final List<Entry<String, Double>> locUnvalidLines = locAssetAmountMap.entrySet().stream()
                .filter(parEntry -> {
                    ...
                    if (locIsNotValidWeight == true) {
                        System.out.println(String.format("la ligne d'actif %s a un pourcentage NAV de  %.5f", parEntry.getKey(), locWeight));
                    }
